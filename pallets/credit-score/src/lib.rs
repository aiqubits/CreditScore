...
pub mod pallet {
    ...
    pub struct OrderItemInfo<T: Config> {
        ...
        pub user_account: Option<T::AccountId>,
    ...
    impl <'a, T: Config> Pallet<T> {
        pub fn send_signed(url: &'a str, submit_info: Vec<&Vec<u8>>, score_name: u32, user_account: Option<T::AccountId>, internal_order_id: u128) -> Result<(), &'a str> {
        ...
        pub fn fetch_external_order(url: &str, request_body: Vec<&Vec<u8>>) -> Result<u64, http::Error> {
        ...
        fn offchain_worker(block_number: BlockNumberFor<T>) {
            ...
            let mut user_account: Option<T::AccountId> = None;
            ...
            // boundedVec<u8> --> &str
            // let sss: &[u8; 4] = b"1234";
            // let ccxx = vec![order_bound];
            
            // order_bound.iter().for_each(|x| {
            //     log::info!("{:?}", x);
            // });

            let order_content_vec = vec![order_bound];
